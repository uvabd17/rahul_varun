package com.linkedinlite.job.security;
...
import com.linkedinlite.job.repository.UserSessionRepository;
...
public class SessionFilter extends OncePerRequestFilter {
        ...
        try {
            ...
            if (sid != null && !sid.isBlank()) {
                sessionRepo.findBySessionIdAndStatus(sid, SessionStatus.ACTIVE)
                        .flatMap(s -> userRepo.findById(s.getUserId()).map(u -> build(sid, s, u)))
    ...
    private SessionContext build(String sid, UserSession s, User u) {
        return SessionContext.builder()
                .sessionId(sid).userId(s.getUserId()).role(u.getRole())
                .build();
    }
}
